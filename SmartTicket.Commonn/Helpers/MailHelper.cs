...
namespace SmartTicket.Commonn.Helpers
{
    class MailHelper
    {
        ...
        //mail atan metodunu yaz config helperdan faydalan bilgileri o çeksin

        public void BuildEmailTemplate(int id)
        {
            string body = System.IO.File.ReadAllText(HostingEnvironment.MapPath("~/Account/") + "ConfirmEmail" + ".cshtml");
            var regInfo = us.Find(x => x.Id == id);
            var url = "http://localhost:50276/"+"Account/ConfirmEmail?id="+id;
        }
    }
}
